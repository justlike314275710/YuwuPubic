...
- (void)p_setUI {
    ...
    NSString *completeBtnText =NSLocalizedString(@"completed", @"完成");
    ...
    NSString *k_payWayLabText = NSLocalizedString(@"payment_method", @"付款方式");
...
- (UILabel *)payStateLab {
    if (!_payStateLab) {
        ...
        NSString *text = @"";
        if (self.state == payScuess) {
            text = NSLocalizedString(@"payment_success", @"支付成功");
            _payStateLab.textColor = UIColorFromRGB(38,76,144);
        } else if(self.state == payFailure) {
            text = NSLocalizedString(@"payment_failure", @"支付失败");
            _payStateLab.textColor = UIColorFromRGB(189, 8, 8);
        } else {
            text = NSLocalizedString(@"payment_cancel", @"支付取消");
            _payStateLab.textColor = UIColorFromRGB(189, 8, 8);
        }
...
- (UILabel *)payWayLab {
    if (!_payWayLab) {
        ...
        if ([self.info.payment isEqualToString:@"WEIXIN"]) {
            text = NSLocalizedString(@"WeChat", @"微信");
        } else {
            text = NSLocalizedString(@"Alipay", @"支付宝");
        }
