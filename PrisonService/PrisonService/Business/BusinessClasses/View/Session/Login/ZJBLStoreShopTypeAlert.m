...
- (void)initUI {
    self.alertView.frame = CGRectMake(20, ([UIScreen mainScreen].bounds.size.height-alertHeight)/2.0+10, [UIScreen mainScreen].bounds.size.width-40, alertHeight);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ...
        if (indexPath.row == self.selectedPrisonerIndex) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row!=self.selectedPrisonerIndex) {
        self.selectedPrisonerIndex=indexPath.row;
    }
   // self.selectedPrisonerIndex=indexPath.row;
    [self closeAction];
}
